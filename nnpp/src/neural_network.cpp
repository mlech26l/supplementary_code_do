...
void NeuralNetwork::AddExcitatorySynapse(int src, int dest, float weight) {
  neurons[dest].AddSynapse(weight, Excitatory, src);
}
void NeuralNetwork::AddInhibitorySynapse(int src, int dest, float weight) {
  neurons[dest].AddSynapse(weight, Inhibitory, src);
}
void NeuralNetwork::AddGapJunction(int src, int dest, float weight) {
  neurons[dest].AddSynapse(weight, GapJunction, src);
  neurons[src].AddSynapse(weight, GapJunction, dest);
}
...
void NeuralNetwork::SetCmOf(int src, float cm) {
  neurons[src].Initialize(V_Leak, cm);
}
